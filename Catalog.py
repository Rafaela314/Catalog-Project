...
from catalog_database_setup import Base, Category, HouseItem, User, engine
...
def categoryItemsJSON(category_id):
    ...
    items = session.query(HouseItem).filter_by(category_id=category_id).all()
    return jsonify(HouseItems=[i.serialize for i in items])


@app.route('/catalog/<int:category_id>/item/<int:item_id>/JSON')
def detailedItemsJSON(category_id, item_id):
    House_Item = session.query(HouseItem).filter_by(id=item_id).one()
    return jsonify(House_Item=House_Item.serialize)
...
def categoryItems(category_id):
    ...
    items = session.query(HouseItem).filter_by(
...
def newItem(category_id):
    ...
    if login_session['user_id'] != category.user_id:
        return "(<script>function myFunction() {alert('You are not '\
                'authorized to add items in this category.'\
                'Please create'\
                'your own category in order to add '\
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash(
            'New House %s item Successfully Created created!' %
            (newItem.name))
...
def editItem(category_id, item_id):
    if 'username' not in login_session:
        return redirect('/login')
    editedItem = session.query(HouseItem).filter_by(id=item_id).one()
    category = session.query(Category).filter_by(id=category_id).one()
    if login_session['user_id'] != category.user_id:
        return "(<script>function myFunction() {alert('You are not'\
                 'authorized to edit items in this category.'\
                 'Please create'\
                 'your own category in order to edit '\
                 'items.');}</script><body onload='myFunction()'>"
    if request.method == 'POST':
        if request.form['name']:
        ...
        session.commit()
        flash("House Item Successfully Edited")
        return redirect(url_for('categoryItems', category_id=category_id))
    else:
...
def deleteItem(category_id, item_id):
    if 'username' not in login_session:
        return redirect('/login')
    category = session.query(Category).filter_by(id=category_id).one()
    itemToDelete = session.query(HouseItem).filter_by(id=item_id).one()
    if login_session['user_id'] != category.user_id:
        return "(<script>function myFunction() {alert('You are not'\
                'authorized to delete items in this category.'\
                'Please create'\
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash('House Item Successfully Deleted')
        return redirect(url_for('categoryItems', category_id=category_id))
    else:
